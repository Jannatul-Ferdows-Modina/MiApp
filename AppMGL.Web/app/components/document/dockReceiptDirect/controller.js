...
define(["app"], function (app) {
    ...
    var injectParams = ["$scope", "$filter", "$timeout", "$routeParams", "$uibModal", "localStorageService", "NgTableParams", "ngNotifier", "dockReceiptDirectService"];
    ...
    var controller = function ($scope, $filter, $timeout, $routeParams, $uibModal, localStorageService, NgTableParams, ngNotifier, entityService) {
        ...
        //#region General
        var lastAction = "";
        $scope.page = appUrl.dockReceiptDirect;
        $scope.tabs = appUrl.dockReceiptDirect.tabs;
        $scope.$parent.pageTitle = "Dock Receipt Direct";
        $scope.$parent.breadcrumbs = ["Shipment", "Trucking", "Dock Receipt Direct"];
        ...
        var getDockReceipt = function () {
            ...
            //}
            var params = {
                DocumentCommonId: $scope.entity.docCommId,
            ...
            entityService.getDockReceipt(params).then(
                function (output) {
                    ...
                    $scope.getCities($scope.document.custStateId);
                    $scope.document.custCityId = $scope.document.custCityId;
        ...
        var saveDocument = function () {
            ...
            $scope.document.SitId= $scope.$parent.selectedSiteId;
        ...
        $scope.lookups = { departmentTypes: [], copyReceiptTypes: [], viewDockReceipts: [], filterFields: [], consignedToTypes: [], lgvwstates: [] };
        ...
        $scope.callCompanyModal = function (type) {

            var compid = 0;
            if (type == 'CONSIGNEE')
                compid = $scope.document.consignedToId;
            if (type == 'exporter')
                compid = $scope.document.exporterId;
            if (type == 'CUSTOMER')
                compid = $scope.document.customerId;
            ...
            modalInstance.result.then(
                function (output) {
                    if (output.resultId == 1001) {
                        ...
                        if (type == 'exporter') {
                            $scope.document.customerId = output.data.contactID;
                            $scope.document.exporterAddress = output.data.address;
                        }
                        if (type == 'CUSTOMER') {
                            $scope.document.exporterId = output.data.contactID;
                            $scope.document.custAddressLine1 = output.data.address;
                        }
                    }
                },
                function (output) {
    ...
    app.register.controller("dockReceiptDirectController", controller);
