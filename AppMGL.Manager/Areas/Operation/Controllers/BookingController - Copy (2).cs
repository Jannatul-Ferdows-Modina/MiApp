    ...
    public class BookingController : BaseController<BookingListDTO, BookingListRepository, USP_GET_ENQUIRY_LIST_Result>
    {
        ...
        public ActionResult Save_EES_Submit(AppMGL.DTO.Report.EESSubmission objees)
        {
            try
            {
               ...
               // if (objees.etype == "N")
               // { 
                    //refreshfileName = objees.AesFileName;
                   // objees.AesFileName = "";
              //  }

                if (objees.isFreeze != "1")
                {
                    ...
                    if(result.Count>0)
                    {
                        if (!string.IsNullOrEmpty(objees.AesFileName)  && result[0].isuploaded == "0")
                        {
                            fileName = objees.AesFileName;
                        }
                       else if (!string.IsNullOrEmpty(objees.AesFileName) && result[0].isuploaded!= "0" && result[0].isdownloaded == "0")
                        {
                            fileName = objees.AesFileName;
                        }
                        else if (!string.IsNullOrEmpty(objees.AesFileName) && result[0].isuploaded != "0" && result[0].isdownloaded!= "0" && string.IsNullOrEmpty(result[0].OriginalITN))
                        {
                    ...
                    //if (!string.IsNullOrEmpty(objees.AesFileName) && objees.etype != "N" && objees.isuploaded == "1" && objees.AESTYPE == "A")
                    //{
                    //    string[] t = objees.AesFileName.Split('_');
                    //    seq = Convert.ToInt32(t[2].ToString().Split('.')[0].ToString());
                    //    seq = seq + 1;
                    ...
                    //        fileseq = seq.ToString();
                    //}
                    //else if (!string.IsNullOrEmpty(refreshfileName) && objees.etype == "N" && objees.isuploaded == "0")
                    //{
                    //    string[] t = refreshfileName.Split('_');
                    //    seq = Convert.ToInt32(t[2].ToString().Split('.')[0].ToString());
                    //    seq = seq + 1;
                    //    if (seq < 10)
                    //        fileseq = "0" + seq.ToString();
                    ...
                    //    fileName = objees.shipmentNumber + "aes_input_" + fileseq + ".txt";
                    //}
                    //if (!string.IsNullOrEmpty(objees.AesFileName) && (objees.isuploaded == "0"))
                    //{
                    //    fileName = objees.AesFileName;
                    //}
                        ...
                        for (int i = 0; i < objees.EquipmentLineDetail.Length; i++)
                        {
                            if (objees.EquipmentLineDetail[i].EquipmentNumber != "")
                            {
                                object[] objLine = new object[7]
                                 ...
                                 new SqlParameter("@DocumentCommonId", objees.EquipmentLineDetail[i].DocumentCommonId?? Convert.DBNull),
                    ...
                    if (objees.VinDetail.Length > 0)
                    {
                        for (int i = 0; i < objees.VinDetail.Length; i++)
                        {
                            if (objees.VinDetail[i].VINNumber != "")
                            {
                                ...
                                _context.ExecuteQuery("EXEC dbo.AES_VINDetail_SP @VIN,@VINNumber,@VehicleTitleNum,@VehicleTitleState,@SiteId,@DocumentCommonId,@AesId,@Line_No", objvin).ToList();
        ...
        public virtual ActionResult GetExportRegisterDetailEES(BookingListDTO objBooking)
        {
            try
            {
                Logger.WriteInfo("getExportRegisterDetailEES 1");
                //List<BookingListDTO> result = _context.ExecuteQuery<BookingListDTO>("EXEC dbo.USP_LG_BOOKING_GET_EXPORT_REGISTER_DETAILS @DOCUMENTCOMMONID,@SIT_ID",
                ...
                Logger.WriteInfo("getExportRegisterDetailEES 2");
                //List<SqlParameter> list2 = new List<SqlParameter>
                //{
                //    new SqlParameter("Type", "HBL"),
                //    new SqlParameter("DocumentCommonId", objBooking.DocumentCommonID)
                //};
                //List<CommodityLine> list3 = _context.ExecuteQuery<CommodityLine>(DocumentCommonQuery.GetCommodityDetail, list2.ToArray()).ToList();
                List<SqlParameter> list2 = new List<SqlParameter>
                {
               ...
               using (SftpClient client = new SftpClient(_FTPHost, 22, FTPUser, FTPPassword))
                ...
                foreach (string s in finalstring)
                {
                    if (s.StartsWith("ES"))
                    {
                        ...
                        if (s.StartsWith("ES1972") || s.StartsWith("ES1974") || s.StartsWith("ES1978") || s.StartsWith("ES1982"))
                        {
                            int len = s.IndexOf("X");
                ...
                if (File.Exists(phypath))
                {
                    ...
                    foreach (string s in finalstring)
                    {
                        if (s.StartsWith("ES"))
                        {
                            ...
                            if (s.StartsWith("ES1974") || s.StartsWith("ES1978") || s.StartsWith("ES1982"))
                            {
                                int len = s.IndexOf("X");
                                itanno = s.Substring(len, 15);
                            }
        ...
        //[System.Web.Http.HttpGet]
        //public ActionResult PendingContainerList(int id)
        //{
        //    try
        //    {
        //        List<ContainerManagementDTO> result = _context.ExecuteQuery<ContainerManagementDTO>("EXEC dbo.LG_PendingContainerListSP @SITEID",
        //         new SqlParameter("SITEID", id)).ToList();
        ...
        public ActionResult SaveStuffing(ConsolidateBookingDTO obtcont)
        {
            try
            {
                ...
                foreach (QuotationListDTO objquot in obtcont.QuotationDetail)
                {
                     objResult = _context.ExecuteQuery<int>("EXEC dbo.LG_ContainerStuffingQuotationSP @BookedId,@QuotationId,@ContainerNo",
                        new SqlParameter("BookedId", obtcont.BookedId),
                        new SqlParameter("QuotationId", objquot.QuotationID),
        ...
        public virtual ActionResult GetStuffingDetail(ConsolidateBookingDTO objcont)
        {
            try
            {
                ...
                List<QuotationListDTO> list3 = _context.ExecuteQuery<QuotationListDTO>("EXEC dbo.LG_StuffingQuotationDetail @BookedId,@Mode", list2.ToArray()).ToList();
        ...
        public ActionResult PendingQuotationList(int id)
        {
            try
            {
                var result = _context.ExecuteQuery<QuotationListDTO>("EXEC dbo.LG_PendingQuotationSP @SITEID", new SqlParameter("SITEID", id)).ToList();
                return AppResult(result, 1, "", EnumResult.Success);
            }
            catch (Exception ex)
            {
        ...
        [System.Web.Http.HttpPost]
        public ActionResult GetPendingStuffingList(ListParams listParams)
        {
            try
            {
                var searchCriteria = JsonConvert.DeserializeObject<Dictionary<string, string>>(listParams.Filter);
                var sortOrder = JsonConvert.DeserializeObject<Dictionary<string, string>>(listParams.Sort);
                int TotalRows = 0;
                List<ContainerStuffingDTO> result = _context.ExecuteQuery<ContainerStuffingDTO>("EXEC dbo.LG_PendingStuffingListSP @PAGENO, @PAGESIZE,@OPTIONVALUE,@SEARCHVALUE,@SORTCOLUMN,@SORTORDER,@SITEID",
                        new SqlParameter("PAGENO", listParams.PageIndex),
                        new SqlParameter("PAGESIZE", listParams.PageSize),
                        new SqlParameter("OPTIONVALUE", searchCriteria["optionValue"]),
                        new SqlParameter("SEARCHVALUE", searchCriteria["seachValue"]),
        ...
        public ActionResult GetShiperList(ListParams listParams)
        {
            try
            {
                ...
                var sortOrder = JsonConvert.DeserializeObject<Dictionary<string, string>>(listParams.Sort);
                int TotalRows = 0;
                List<DocumentationListDTO> result = _context.ExecuteQuery<DocumentationListDTO>("EXEC dbo.USP_SLI_FORM_GET_LIST @PAGENO, @PAGESIZE,@OPTIONVALUE,@SEARCHVALUE,@DOS_ID,@SORTCOLUMN,@SORTORDER,@DASHBOARD_FILTER,@SIT_ID",
                        new SqlParameter("PAGENO", listParams.PageIndex),
                        new SqlParameter("PAGESIZE", listParams.PageSize),
                        new SqlParameter("OPTIONVALUE", searchCriteria["optionValue"]),
                        new SqlParameter("SEARCHVALUE", searchCriteria["seachValue"]),
                ...
                object[] obj = new object[75]
                 {
                     ...
                     new SqlParameter("IsRoutedTransaction", objees.isRoutedTransaction ),
                     new SqlParameter("IsTIB", objees.isTIB ),
                     new SqlParameter("IsHazardousMaterial", objees.isHazardousMaterial ),
