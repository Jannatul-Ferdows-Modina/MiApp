...
define(["app"], function (app) {
    ...
    var injectParams = ["$scope", "$filter", "$timeout", "$location", "$routeParams", "$uibModal", "localStorageService", "NgTableParams", "ngNotifier", "processInvoiceReportService"];
    ...
    var controller = function ($scope, $filter, $timeout, $location, $routeParams, $uibModal, localStorageService, NgTableParams, ngNotifier, entityService) {
        ...
        //#region General
        $scope.page = appUrl.processInvoiceReport;
        $scope.tabs = appUrl.processInvoiceReport.tabs;
        ...
        $scope.showNextActionRemarksModel = function (documentCommonID) {
            ...
            //start model
                     var modalInstance = $uibModal.open({
                         ...
                         keyboard: false,
                         size: "lg",
                         templateUrl: "app/components/report/processInvoiceReport/viewExpensesReport.html",
        ...
        $scope.showExpensesReport = function (documentCommonID) {
            $scope.entities = {};
            $scope.entities.documentCommonID = documentCommonID;
            $scope.entities.siteId = $scope.$parent.selectedSiteId;

            entityService.getExpensesReportDetails($scope.entities).then(
                 function (output) {
                     if (output.data.resultId == 2005) {
                         ngNotifier.showError($scope.authentication, output);
                         $scope.logOut()
                     ...
                     var modalInstance = $uibModal.open({
                         ...
                         controller: function ($scope, $timeout, $uibModalInstance, requestData) {
                             $scope.reportTable = [];
                             if (requestData.documentEntity != null && requestData.documentEntity.length != 0) {
                                 $scope.reportTable = requestData.documentEntity;
                                 $scope.reportTable = new NgTableParams(
                                          {
                                              ...
                                              counts: [],
                                              getData: function (params) {
                                                  params.total(requestData.documentEntity.length);
                                                  return requestData.documentEntity;
                                              }
                                          });
                             }
        ...
        $scope.viewQuotationCharges = function (quotationId) {
    ...
    app.register.controller("processInvoiceReportController", controller);
