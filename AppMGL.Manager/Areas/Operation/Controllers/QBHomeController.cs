...
namespace AppMGL.Manager.Areas.Operation.Controllers
{
    ...
    public class QBHomeController : Controller
    {
        ...
        public async Task<ActionResult> Index()
        {
            ...
            if (discoveryUrl != null && QBAPPController.clientid != null && QBAPPController.clientsecret != null)
            {
            ...
            {
                //Authorize endpoint
                QBAPPController.authorizeUrl = doc.AuthorizeEndpoint;

                //Token endpoint
                QBAPPController.tokenEndpoint = doc.TokenEndpoint;

                //Token Revocation enpoint
                QBAPPController.revocationEndpoint = doc.RevocationEndpoint;

                //UserInfo endpoint
                QBAPPController.userinfoEndpoint = doc.UserInfoEndpoint;

                //Issuer endpoint
                QBAPPController.issuerEndpoint = doc.Issuer;

                //JWKS Keys
                QBAPPController.keys = doc.KeySet.Keys;
            }

            //Get mod and exponent value
            foreach (var key in QBAPPController.keys)
            {
                if (key.N != null)
                {
                    //Mod
                    QBAPPController.mod = key.N;
                }
                if (key.N != null)
                {
                    //Exponent
                    QBAPPController.expo = key.E;
                }
            }
        ...
        private string GetAuthorizeUrl(string scope)
        {
            ...
            var request = new AuthorizeRequest(QBAPPController.authorizeUrl);

            string url = request.CreateAuthorizeUrl(
               clientId: QBAPPController.clientid,
               responseType: OidcConstants.AuthorizeResponse.Code,
               scope: scope,
               redirectUri: QBAPPController.redirectUrl,
               state: state);
