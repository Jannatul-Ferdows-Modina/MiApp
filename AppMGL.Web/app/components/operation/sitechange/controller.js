...
define(["app"], function (app) {
    ...
    var injectParams = ["$scope", "$filter", "$timeout", "$routeParams", "$uibModal", "localStorageService", "NgTableParams", "ngNotifier", "sitechangeService"];
    ...
    var controller = function ($scope, $filter, $timeout, $routeParams, $uibModal, localStorageService, NgTableParams, ngNotifier, entityService) {
        ...
        //#region General
        $scope.page = appUrl.sitechange;
        $scope.tabs = appUrl.sitechange.tabs;
        $scope.$parent.pageTitle = "Site Change";
        $scope.$parent.breadcrumbs = ["Shipment", "Quotation", "Site Change"];
        //$scope.departments = [];
        $scope.enquiryID;
        $scope.deleteRemarks = "";
        ...
        $scope.setLookups = function (source, lookup, output, index) {
            ...
            if (lookup == "SIPLContact") {
                ...
                if (output.data[0].cityName != null) {
                    pickupaddress = output.data[0].cityName;
                } else { pickupaddress = ""; }
        ...
        $scope.bookinglistTable = new NgTableParams(
       {
           ...
           getData: function (params) {
               ...
               var dataitems = entityService.list_site(listParams).then(
                   function (output) {
                       $scope.validateUser(output);
                       $scope.items = output.data.data;
                       params.total(output.data.count);
                       setTimeout(function () { $(".btnsite").removeAttr("disabled"); }, 1000);
    ...
    app.register.controller("sitechangeController", controller);
