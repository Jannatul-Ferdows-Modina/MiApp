...
define(["app"], function (app) {
    ...
    var injectParams = ["$scope", "$filter", "$timeout", "$routeParams", "$uibModal", "localStorageService", "NgTableParams", "ngNotifier", "mblService"];
    ...
    var controller = function ($scope, $filter, $timeout, $routeParams, $uibModal, localStorageService, NgTableParams, ngNotifier, entityService) {
        ...
        $scope.page = appUrl.mbl;
        $scope.tabs = appUrl.mbl.tabs;

        $scope.$parent.pageTitle = "MBL";
        $scope.$parent.breadcrumbs = ["Document", "MBL"];
        ...
        var getMBL = function () {
            ...
            entityService.getMBL(params).then(
                function (output) {
        ...
        var saveDocument = function () {
            ...
            entityService.saveMBL($scope.document).then(
                function (output) {
        ...
        $scope.freightTable = new NgTableParams(
        {
            ...
            counts: [],
            getData: function (params) {
                if ($scope.document && $scope.document.freightDetail == undefined)
                {
                    params.total(0);
                    return null;
                }
    ...
    app.register.controller("mblController", controller);
