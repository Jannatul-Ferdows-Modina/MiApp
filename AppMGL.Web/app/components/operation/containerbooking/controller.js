    ...
    var controller = function ($scope, $filter, $timeout, $routeParams, $uibModal, localStorageService, NgTableParams, ngNotifier, entityService) {
        ...
        $scope.setLookups = function (source, lookup, output, index) {
            ...
            if (lookup == "SIPLContact") {
                $scope.entity.companyId = output.data[0].contactID;
                $scope.entity.companyAddress = output.data[0].address;
            }
            ...
            //}
                ...
                if (output.data[0].companyName != null) {
                    pickupaddress = output.data[0].companyName;
                    $scope.entity.pickupLocation = output.data[0].companyName;
                }
                else { pickupaddress = ""; }

                if (output.data[0].cityName != null) {
                    pickupaddress = pickupaddress + ", " + output.data[0].cityName;
                }
                //else { pickupaddress = ""; }

                if (output.data[0].stateName != null) {
                    pickupaddress = pickupaddress + ", " + output.data[0].stateName;
                }// else { pickupaddress = pickupaddress + ","; }


                //$scope.entity.pickupLocationAddress = pickupaddress;
                $scope.entity.pickupLocationAddress = output.data[0].address;
            }
                ...
                if (output.data[0].companyName != null) {
                    returnaddress = output.data[0].companyName;
                    $scope.entity.returnLocation = output.data[0].companyName;
                }
                else { returnaddress = ""; }

                if (output.data[0].cityName != null) {
                    returnaddress = returnaddress + ", " + output.data[0].cityName;
                }
                //else { returnaddress = ""; }
                if (output.data[0].stateName != null) {
                ...
                $scope.entity.returnLocationAddress = output.data[0].address;
            }
                ...
                $scope.entity.originPortId = output.data[0].portId;
            }
            else if (lookup == "discharge") {
                $scope.entity.destinationPortId = output.data[0].portId;
            }
        ...
        $scope.ChangeSite = function (EnquiryNo, EnquiryID, quotationID) {
            var modalInstance = $uibModal.open({
                animation: true,
                size: "md",
                templateUrl: "app/components/operation/enquiry/sitechange.html",
                controller: function ($scope, $timeout, $uibModalInstance, requestData) {
                    $scope.quotationid = requestData.quotationid;
                    $scope.selectedenqid = requestData.selectedenqid;
                    $scope.selectedenqno = requestData.selectedenqno;
                    $scope.selectedsiteid = requestData.selectedsiteid;
                    $scope.allsite = requestData.usersite;
                    $scope.select = function (action) {
                        //$scope.deleteRemarks = deleteRemarks.value;
                        if (action == 'sitechange' && $scope.selectedsiteid == null) {
                            ngNotifier.error("Please select site");
                            return;
                        }
                        var outputData = {}
                        outputData.NewSiteId = $scope.selectedsiteid;
                        outputData.action = action;
                        outputData.selectedenqid = $scope.selectedenqid;
                        outputData.quotationid = $scope.quotationid;
                        outputData.selectedenqno = $scope.selectedenqno;
                ...
                resolve: {
                    requestData: function () {
                        return {
                            usersite: $scope.allsite,
                            selectedsiteid: $scope.$parent.selectedSiteId,
                            selectedenqid: EnquiryID,
                            selectedenqno: EnquiryNo,
            ...
            modalInstance.result.then(
                function (output) {
                    if (output.action == "sitechange") {

                        //entity.remarks = deleteRemarks.value;
                        var objEnquiry = {};
                        objEnquiry.NewSiteId = output.NewSiteId;
                        objEnquiry.EnquiryNo = output.selectedenqno;
                        objEnquiry.EnquiryID = output.selectedenqid;
                        objEnquiry.quotationID = output.quotationid;
                        objEnquiry.SiteId = $scope.$parent.selectedSiteId;
                        objEnquiry.CreatedBy = $scope.$parent.authentication.userId;
                        objEnquiry.IsComplete = 1;
                        entityService.changeSite(objEnquiry).then(
                            function (output) {
                                //$scope.entity = {};
        ...
        $scope.addContainerRow = function () {
            ...
            var isexists = false;
            $scope.containerServiceList.forEach(function (item) {
                if (item.categoryName == $scope.entity.categoryItem.categoryName
                    && item.containerName == $scope.entity.containerTypeItem.name
                    && item.containerTypeID == $scope.entity.containerTypeItem.containerTypeId
            ...
            if (isexists) {
                ngNotifier.error("already exists");
                return;
            }
        ...
        $scope.afterPerformAction = function (source, fromList) {
            ...
            switch (action) {
                case "add":
                    ...
                    $scope.containerServiceList = [];
                    $scope.isDepartmentDisabled = false;
                    break;
                case "edit":
        ...
        $scope.afterGetDetail = function (action) {
            ...
            //if ($scope.entity.bookingCategory == 0) { $scope.entity.bookingCategory = "0"; }
            //else if($scope.entity.bookingCategory ==1) {$scope.entity.bookingCategory = "1";}

           // $scope.documentationStatusList = $scope.lookups.documentationStatus;
        ...
        $scope.pickupTypes = [
            ...
            { optionValue: "8", optionName: "SD/CFS" }
        ];
        $scope.sipl_MailMode = [
        ...
        $scope.filterBookings = function () {
            ...
            if (dashboardOption != null) {
                if (dashboardOption == 'BTDU') {
                    $scope.dashboardOption = "BTDU";
                }

                if (dashboardOption == 'BTDF') {
        ...
        //#region Methods
        $scope.consolidateBookingListTable = new NgTableParams(
            {
                page: 1,
                count: 10,
            ...
            getData: function (params) {
                var listParams = {
                    ...
                    var dataitems = entityService.GetConsolidateBookingList(listParams).then(
                    function (output) {
        ...
        $scope.performContainerSearch = function (source, selectOption, searchBox) {
            ...
            $scope.consolidateBookingListTable.reload();
        };
        ...
        $scope.SaveConsolidateBooking = function () {
            ...
            if ($scope.entity.bookingDate == null) {
                ngNotifier.error("Please enter Booking Date.");
                return;
            }
            if ($scope.entity.originPort == null) {
                ngNotifier.error("Please select port of origin.");
                return;
            }
            if ($scope.entity.destinationPort == null) {
            ...
            if ($scope.entity.ets < $scope.entity.portcutoffDate) {
                ngNotifier.error("Please select Estimated Time of Sailing if grater then Cargo Cut off Date");//ESTIMATED TIME OF SAILING->=CARGO CUT OFF DATE
                return true;
            }

            if ($scope.entity.ets < $scope.entity.earliestPickUpDate) {
                ngNotifier.error("Please select Estimated Time of Sailing is Grater then or Equal to Earliest pick up date");//ESTIMATED TIME OF ARRIVAL>=ESTIMATED TIME OF SAILING
                return true;
            }


            if ($scope.entity.earliestPickUpDate > $scope.entity.portcutoffDate) {
                ngNotifier.error("Please select Earliest pick up date is less then or equal to Cargo cut off Date");//EARLIEST CONT. PICKUP DATE<= CARGO CUT OFF DATE
                return true;
            }
            if ($scope.entity.earliestPickUpDate > $scope.entity.erd) {
                ngNotifier.error("Please select Earliest Return Date is grater then Earliest pick up date ");//EARLIEST CONT. RETURN DATE>= EARLIEST CONT. PICKUP DATE
            ...
            entityService.SaveConsolidateBooking($scope.entity).then(
                function (output) {
        ...
        $scope.getConsolidateBookingDetail = function (action, id) {
            ...
            $scope.entity.userId = $scope.$parent.authentication.userId;
            if ($scope.entityId > 0) {
                entityService.getConsolidateBookingDetail($scope.entity).then(
                    function (output) {
        ...
        $scope.deleteConsolidateBooking = function (id) {
            $scope.entityId = id;
            $scope.entity.bookedId = parseFloat(id);
            if ($scope.entityId > 0) {
                entityService.deleteConsolidateBooking($scope.entity).then(
                    function (output) {
                        if (output.data.resultId == 2005) {
                            ngNotifier.showError($scope.authentication, output);
                            $scope.logOut()
        ...
        $scope.performBookingAction = function (source, fromList) {
            ...
            if (fromList) {
                $scope.getConsolidateBookingDetail(action, source.currentTarget.attributes["entityId"].value);
            ...
            switch (action) {
                ...
                case "save":
                    ...
                    break;
                case "cancel":
                    $scope.getConsolidateBookingDetail('viewDetail', $scope.entity.containerId);
                    lastAction = "";
                    break;
                case "delete":
                    remove();
        ...
        //$scope.getLatestBookingNo = function () {

        //    var entites = {};
        //    entites.siteId = $scope.$parent.selectedSiteId;
        //    entites.departmentID = $scope.entity.departmentID;
        ...
        $scope.callPortModal = function (source) {
            ...
            var sourceId;
            if (source == 'origin') {
                if ($scope.entity.originPortId != null)
                    sourceId = $scope.entity.originPortId;
            }
            else {
                if ($scope.entity.destinationPortId != null) {
                    sourceId = $scope.entity.destinationPortId;
                }
            }
            var modalInstance = $uibModal.open({
                animation: false,
                    ...
                    if (output.resultId == 1001) {
                        ...
                        if (source == 'origin') {
                            if (output.data.portId != null) {
                                $scope.entity.originPortId = output.data.portId;
                            }
                            if (output.data.name != null) {
                                $scope.entity.originPort = output.data.name;
                            }
                        }
                        else {
                            if (output.data.portId != null) {
                                $scope.entity.destinationPortId = output.data.portId;
                            }
                            if (output.data.name != null) {
                                $scope.entity.destinationPort = output.data.name;
                            }
                        }
                    }
                },
        ...
        $scope.callCompanyModal = function (source) {
            ...
            modalInstance.result.then(
                function (output) {
                    if (output.resultId == 1001) {
                        $scope.entity.companyId = output.data.contactID;
                        $scope.entity.companyAddress = output.data.address;
                    }
                },
                function (output) {
                    ngNotifier.logError(output);
