    ...
    var controller = function ($scope, $filter, $timeout, $routeParams, $uibModal, localStorageService, NgTableParams, ngNotifier, entityService) {
        ...
        $scope.bookinglistTable = new NgTableParams(
       {
        ...
        $scope.saveDocumentDetail = function (source, fromList) {
            ...
            if ($scope.entity.docRequiredids != null) {
                var docreq = "";
                for (var i = 0; i < $scope.entity.docRequiredids.length; i++) {
                    if (i == 0) {
                        docreq = $scope.entity.docRequiredids[i];
                    }
                    else {
                        docreq = docreq + "," + $scope.entity.docRequiredids[i];
                    }
                }
                $scope.entity.docRequired = docreq;
            }
        ...
        var cancelDocumentCopy = function (documentCommonID , msg) {
            var entity = {};
        ...
        $scope.showDocumentDetail = function (action, documentCommonID) {
            ...
            entityService.getDocumentDetail($scope.entity).then(
                 function (output) {
                    ...
                    var temp1 = new Array();
                    if ($scope.entity.docRequired != null) {
                        temp1 = $scope.entity.docRequired.split(",");
                        for (var a in temp1) {
                           // temp1[a] = parseInt(temp1[a]);
                            temp1[a] = temp1[a];
                        }
                        $scope.entity.docRequiredids = temp1;
                    }
        ...
        $scope.CopyEnquiry = function (source, fromList, documentCommonID, type, quotationID, enquiryId) {
            var msg = '';

            if (type == 'e') {
                msg = 'Are you sure you want to create Enquiry?'
            }
            else {
                msg = 'Are you sure you want to create Quatation?'
            }

            ngNotifier.confirm(msg, null, function () {             

                var obj = {};
                obj.documentCommonID = documentCommonID;
                obj.quotationID = quotationID;
                obj.enquiryID = enquiryId;
                obj.etype = type;
                obj.CreatedBy = $scope.$parent.authentication.userId;
                obj.SiteId = $scope.$parent.selectedSiteId;
                entityService.CopyEnquiry(obj).then(
                 function (output) {
                     ...
                     var msg = "";
                     if (type == 'e') {
                         msg = "Your Enquiry No. is " + data.enquiryNo + " successfully created .Are you want to cancel original booking?.";
                     } else {
                         msg = "Your Enquiry No. is " + data.enquiryNo + " and Quatation No. " + "NRA" + data.quotationNo + " successfully created. Are you want to cancel original booking?";
                     }

                     cancelDocumentCopy(documentCommonID, msg);
