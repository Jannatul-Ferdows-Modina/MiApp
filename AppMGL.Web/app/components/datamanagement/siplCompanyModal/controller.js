...
define(["app"], function (app) {
   ...
   //     $scope.lookups.categories = [];
        ...
        $scope.categories = [];
        $scope.companyID = (requestData.companyID) ? requestData.companyID : 0;
        ...
        if ($scope.companyID != undefined && $scope.companyID != '' && $scope.companyID != null) {

            entityService.getContactDetail($scope.companyID).then(
                function (output) {
                    if (output.data.resultId == 2005) {
                        ngNotifier.showError($scope.authentication, output);
                        $scope.logOut()
        ...
        $scope.selectTypeaheadAES = function ($item, $model, $label, source, stype) {
            ...
            if (stype == "companyName") {
                $scope.entity.companyName = $item.name;
                
            }
        };
        $scope.compairdata = function () {
            var modalInstance = $uibModal.open({
                animation: false,
                backdrop: "static",
                keyboard: false,
                size: "lg",
                templateUrl: "app/components/report/quickbooksync/detail.html",
                controller: "quickbooksyncController",
                resolve: {
                    requestData: function () {
                        return {
                            qbid: ($scope.entity.qbid || 0),
                            contactId: ($scope.entity.contactID || 0)
                        };
                    }
                }
            });
        ...
        $scope.SaveCompairData = function (type) {
            $scope.entities = {};
            if (type == "App")
                $scope.entities.isquickbook = 1;
            else
                $scope.entities.isquickbook = 0;
            ...
            if (type == "App") {
                if ($scope.entity.companyName == "" || $scope.entity.companyName == null) {
                    ...
                    $scope.entities.companyName = $scope.entity.companyName;
                }
                ...
                if ($scope.entity.contactPersonFirstName == "" || $scope.entity.contactPersonFirstName == null) {
                    alert("Please enter Contact Person");
                    return false;
                }
                else {
                    $scope.entities.contactPerson = $scope.entity.contactPersonFirstName + " " + $scope.entity.contactPersonLastName;
                }
                if ($scope.entity.telNo == "" || $scope.entity.telNo == null) {
                 ...
                 $scope.entities.telNo = $scope.entity.telNo;
                }
                $scope.entities.email = $scope.entity.email;
                $scope.entities.countryName = $scope.entity.countryName;
                $scope.entities.address = $scope.entity.address;
                $scope.entities.zipCode = $scope.entity.zipCode;
                $scope.entities.City = $scope.entity.city;
                $scope.entities.qbid = 0;
                
            }

            if ($scope.companyID > 0) {
                entityService.saveCompairData($scope.entities).then(
                    function (output) {
