...
define(["app"], function (app) {
    ...
    var injectParams = ["$scope", "$filter", "$timeout", "$location", "$routeParams", "$uibModal", "localStorageService", "NgTableParams", "ngNotifier", "activityDueReportService"];
    ...
    var controller = function ($scope, $filter, $timeout, $location, $routeParams, $uibModal, localStorageService, NgTableParams, ngNotifier, entityService) {
        ...
        //#region General
        $scope.page = appUrl.activityDueReport;
        $scope.tabs = appUrl.activityDueReport.tabs;
     ...
     $scope.reportTable = new NgTableParams(
      {
          ...
          getData: function (params) {
              ...
              var dataitems = entityService.getActivityDueReportList(listParams).then(
                  function (output) {
     ...
     $scope.performSearch = function (source, fromDate, toDate) {
     ...
     $scope.exportActivityDueReport = function (source, fromDate, toDate) {

            var action = source.currentTarget.attributes["action"].value;
            if ((toDate == null || toDate == '') && (fromDate != null && fromDate != '')) {
                ngNotifier.error("Please enter valid From Date");
                return;
            }
           
            if ((fromDate == null || fromDate == '') && (toDate != null && toDate != '')) {
                    ngNotifier.error("Please enter valid To Date");
                    return;               
            }           
            ...
            entityService.exportActivityDueReport(reportParams).then(
                function (output) {
    ...
    app.register.controller("activityDueReportController", controller);
