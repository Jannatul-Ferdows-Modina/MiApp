...
define(["app"], function (app) {
    ...
    var injectParams = ["$scope", "$filter", "$timeout", "$routeParams", "$uibModal", "localStorageService", "NgTableParams", "ngNotifier", "bookingService"];
    ...
    var controller = function ($scope, $filter, $timeout, $routeParams, $uibModal, localStorageService, NgTableParams, ngNotifier, entityService) {
        ...
        //#region General
        $scope.page = appUrl.booking;
        $scope.tabs = appUrl.booking.tabs;
        $scope.$parent.pageTitle = "Booking To Do List";
        $scope.$parent.breadcrumbs = ["Shipment", "Booking", "Booking To Do"];
        //$scope.departments = [];
        $scope.enquiryID;
        $scope.deleteRemarks = "";
        ...
        $scope.UpdateContainerSizes = function (selectedcategoryItem) {
            ...
            $scope.lookups.containerSizes.forEach(function (item) {
                if ($scope.entity.modeOfService == '1') {
                    $scope.containerServiceListDDL.forEach(function (item1) {
                        if (item.containerTypeId == item1.containerTypeID && selectedcategoryItem.categoryName == item1.categoryName) {
        ...
        $scope.bookinglistTable = new NgTableParams(
       {
        ...
        $scope.getLatestBookingNo = function () {
            ...
            var getBookingNo = entityService.getLatestBookingNo(entites).then(
                function (output) {
                    if ($scope.entity.fileNo.indexOf('STR') > -1) {
                        $scope.entity.fileNo = output.data.data.fileNo + 'STR';
                    } else {
        ...
        var deleteQuotation = function (enquiryID, quotationID) {
            var entity = {};
            entity.enquiryID = enquiryID;
            entity.quotationID = quotationID;

            ngNotifier.confirm("Are you sure you want to DELETE Approved Quotation?", null, function () {
                var modalInstance = $uibModal.open({
                    animation: false,
                    backdrop: "static",
                    keyboard: false,
                    size: "lg",
                    templateUrl: "app/components/operation/booking/cancelRemarks.html",
                    controller: function ($scope, $timeout, $uibModalInstance, requestData) {
            ...
            if ($scope.entity.enquiryID > 0 && $scope.entity.quotationID > 0) {
                entityService.getBookingDetail($scope.entity).then(
                     function (output) {
                         ...
                         if ($scope.entity.newQuotationID != 0) {
                             $(".box-footer button[name='btnAdd']").hide();
                             $(".box-header button[name='btnAdd']").hide();                             
                         }
                         else {
                             $(".box-footer button[name='btnAdd']").show();
                             $(".box-header button[name='btnAdd']").show();
                         }

                         if (action == 'newsitedetail')
                         {
                             $(".box-footer button[name='btnAdd']").hide();
                             $(".box-header button[name='btnAdd']").hide();
                         } else
        ...
        $scope.showBookingDetail_QuotationId = function ( quotationID) {

            $scope.onClickTab($scope.tabs[0]);
            viewDetail();
            initControls('edit'); 
            var BookingDetailDTO = {};
            BookingDetailDTO.QuotationID = quotationID ;
            
           
            if (quotationID > 0) {
                entityService.getBookingDetail_BYQuationID(BookingDetailDTO).then(
                     function (output) {
                         if (output.data.resultId == 2005) {
                             ngNotifier.showError($scope.authentication, output);
                             $scope.logOut()
                         ...
                         if ($scope.entity.enquiryContainerServiceDTOList != null) {
                             if ($scope.entity.enquiryContainerServiceDTOList.length > 0) {
                                ...
                                // $scope.containerServiceListDDL = $scope.entity.enquiryContainerServiceDTOList;
                                 $scope.containerServiceList.forEach(function (o) {
                                     total = total + o.quantity
                                 });
                                 $scope.entity.noOfContainer = total;
        ...
        $scope.performBookingAction = function (source, fromList, enquiryID, quotationID, documentCommonID) {
            ...
            switch (action) {
                ...
                case "cancel":
                    ...
                    break;
                case "delete":
                    deleteQuotation(enquiryID, quotationID);
                    //lastAction = "";
                    break;
                case "deleteBatch":
    ...
    app.register.controller("bookingController", controller);
