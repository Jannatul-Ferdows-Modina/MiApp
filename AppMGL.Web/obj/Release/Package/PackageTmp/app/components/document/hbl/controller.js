...
define(["app"], function (app) {
    ...
    var injectParams = ["$scope", "$filter", "$timeout", "$routeParams", "$uibModal", "localStorageService", "NgTableParams", "ngNotifier", "hblService"];
    ...
    var controller = function ($scope, $filter, $timeout, $routeParams, $uibModal, localStorageService, NgTableParams, ngNotifier, entityService) {
        ...
        $scope.page = appUrl.hbl;
        $scope.tabs = appUrl.hbl.tabs;

        $scope.$parent.pageTitle = "HBL";
        $scope.$parent.breadcrumbs = ["Document", "HBL"];
        ...
        var getHBL = function () {
            ...
            entityService.getHBL(params).then(
                function (output) {
                ...
                function (output) {
                    if (output.data && output.resultId == 1001) {
                        if (output.data.exportRef != "" && output.docCommId != "") {
                            $scope.entity.filterFieldName = "miamiRefNo";
        ...
        var saveDocument = function () {
            ...
            entityService.saveHBL($scope.document).then(
                function (output) {
    ...
    app.register.controller("hblController", controller);
