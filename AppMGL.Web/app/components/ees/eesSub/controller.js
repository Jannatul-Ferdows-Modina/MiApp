...
define(["app"], function (app) {
    ...
    var controller = function ($scope, $filter, $timeout, $location, $routeParams, $uibModal, localStorageService, NgTableParams, ngNotifier, entityService) {
        ...
        $scope.isDel = "D";
        $scope.ExpLicVal;
        $scope.hideisfreeze = true;
        $scope.hidesave = false;
        $scope.hidesave1 = 0;
        $scope.HideFreezeRemarks = true;
        $scope.messagelist = "";
        $scope.olddocumentcommanid = "";
        $scope.oldsystemrefno = "";
        $scope.quotationNo = "";
        $scope.bookingNo = "";
        $scope.quotationID = "";
        $scope.enquiryNo = "";
        $scope.oldAesId = "0";
        $scope.oldaesFileName = "";
        $scope.messagelist = "";
        ...
        $scope.addNewLine = function () {
            ...
            $scope.entity.commodityDetail.push({
                isType: "N", exportCode: "OS", scheduleB: "", commodityDescription: "",
                firstQuantity: "", firstUOM: "", secondQuantity: "", secondUOM: "", originofGoods: "",
                valueofGoods: "", shippingWeight: "", eccn: "", licenseTypeCode: "C33", expLic: 'NLR', isGovermentAgency: "Y", documentCommonId: $scope.entity.documentCommonID, siteId: $scope.entity.siteId,
                ctype: $scope.entity.ctype
            });
        ...
        $scope.addCommodities = function (data) {
            ...
            //var ctype = 'A';
            //if ($scope.entity.idd == '0' || $scope.entity.idd == undefined)
            //    ctype = 'A'
            //if ($scope.entity.idd != '0' && $scope.entity.idd != undefined && $scope.entity.isuploaded =="1")
            //    ctype = 'C'
            if ($scope.isaddcomdity == 1) {
                alert("Please add new line first before save line.");
                return;
            }
        ...
        $scope.delCommodities = function (rownum) {
            ...
            if (cind.isType == "O") {
                ...
                $scope.entity.commodityDetail[rownum].ctype = "D";
            }
            else {
        ...
        $scope.selecteditCommodities = function (id) {
            ...
            if (comm.isGovermentAgency == null || comm.isGovermentAgency == "" || comm.isGovermentAgency == undefined)
                $scope.entity.IsGovermentAgency = 'N';
            else
            ...
            if (comm.ctype == "" || comm.ctype == null || comm.ctype == undefined)
                $scope.entity.ctype = "A";
            else
                $scope.entity.ctype = comm.ctype;
        ...
        $scope.callHtsCodeModal = function (ctype) {
            ...
            var modalInstance = $uibModal.open({
                ...
                resolve: {
                    requestData: function () {
                        ...
                        return {
                            htscode: ($scope.entity.ScheduleB || ''),
                            htsid: ($scope.entity.htsCodeId || '')
                        };
                    }
                }
            ...
            modalInstance.result.then(
                function (output) {
                    if (output.resultId == 1001) {
                        ...
                        $scope.entity.FirstUOM = output.data.firstUOM;
                        $scope.entity.SecondUOM = output.data.secondUOM;
                        $scope.entity.CommodityDescription = output.data.description;
                        $scope.entity.htsCodeId = output.data.id;
                        $scope.showVin(output.data.htsNumber, output.data.isvin);
        ...
        $scope.showVin = function (ScheduleB, isvin) {
            if (ScheduleB == "8703.80.0000" || isvin == "1") {
                $("#VIN_section").show();
                $("#VIN_tab_li").show();
            }
            else {
        ...
        $scope.lookups = { siplDepartments: [], miamiBookingStatus: [], countrynames: [], statenames: [], filingOptions: [], modeOfTransports: [], exportInformationCodes: [], uomList: [], htsCode: [], portofExport: [], ddtcitar: [], ddtcusml: [] };
        $scope.searchResult = [];
        ...
        $scope.refreshData = function () {
            var dform = $scope.entity.dataSourceFrom;
            if (dform == '-Select-' || dform == undefined) {
                alert("Please select data source.");
                return true;
            }
           else if (dform == "T") {
                $scope.viewtemplatedata($scope.$parent.selectedSiteId, $scope.entity.documentCommonID);
            }
           else if (dform == "USPPI" || dform == "Ultimate") {
                $scope.viewpartytemplate(dform);
            }
          else {
                $scope.showViewDocumentModel($scope.entity.documentCommonID, dform, 0)
            }
        };

        $scope.showViewDocumentModel = function (documentCommonID, type, Id) {
            if (type == 'N') {
            ...
            entityService.getExportRegisterDetailEES($scope.entities).then(
                function (output) {
                    ...
                    if (output.data.data != null) {
                        ...
                        $scope.entity = object;
                        $scope.entity.id = object.id;
                        $scope.entity.documentCommonID = object.documentCommonID;
                        $scope.entity.shipmentNumber = object.shipmentNumber
                        if (type == '') {
                            $scope.olddocumentcommanid = object.documentCommonID;
                            $scope.oldsystemrefno = object.shipmentNumber;
                            $scope.quotationNo = object.quotationNo;
                            $scope.bookingNo = object.bookingNo;
                            $scope.quotationID = object.quotationID;
                            $scope.enquiryNo = object.enquiryNo;
                            $scope.oldAesId = object.id;
                            $scope.oldaesFileName = object.aesFileName;
                        }
                        if (type == 'T' || type == 'B') {
                            $scope.entity.documentCommonID = $scope.olddocumentcommanid;
                            $scope.entity.shipmentNumber = $scope.oldsystemrefno;
                            $scope.entity.quotationNo = $scope.quotationNo;
                            $scope.entity.bookingNo = $scope.bookingNo;
                            $scope.entity.quotationID = $scope.quotationID;
                            $scope.entity.enquiryNo = $scope.enquiryNo;
                           // $scope.entity.id =$scope.oldAesId;
                        }
                        if (type == 'T' || type == 'B' || type == 'S') {
                            if ($scope.oldAesId != "0") {
                                $scope.entity.id = $scope.oldAesId;
                                $scope.entity.aesFileName = $scope.oldaesFileName;
                        ...
                        var uSPPIAddress1 = "";
                        var uSPPIAddress2 = "";
                        if (object.usppiAddressLine1 != undefined && object.usppiAddressLine1 != "" && object.usppiAddressLine1 != null) {
                            if (object.usppiAddressLine1.length > 32) {
                                uSPPIAddress1 = object.usppiAddressLine1.toString().substring(0, 31)
                                uSPPIAddress2 = object.usppiAddressLine1.toString().substring(32, object.usppiAddressLine1.length)
                            }
                            else {
                                uSPPIAddress1 = object.usppiAddressLine1;
                            }
                        }
                        if (object.usppiAddressLine2 != undefined && object.usppiAddressLine2 != "" && object.usppiAddressLine2 != null) {
                            if (object.usppiAddressLine2.length > 32) {
                                uSPPIAddress2 = object.usppiAddressLine2.toString().substring(0, 31)

                            }
                            else {
                                uSPPIAddress2 = object.usppiAddressLine2;
                            }
                        }
                        else {
                            if (uSPPIAddress2 != undefined && uSPPIAddress2 != "" && uSPPIAddress2 != null) {
                                if (uSPPIAddress2.length > 32) {
                                    uSPPIAddress2 = uSPPIAddress2.toString().substring(0, 31)

                                }

                            }
                        }
                        $scope.entity.uSPPIAddressLine1 = uSPPIAddress1;
                        $scope.entity.uSPPIAddressLine2 = uSPPIAddress2;
                       ...
                       // $scope.entity.uSPPIStateCode = object.usppiStateCode;
                        ...
                        var ultimateAddress1 = "";
                        var ultimateAddress2 = "";
                        if (object.ultimateAddressLine1 != undefined && object.ultimateAddressLine1 != "" && object.ultimateAddressLine1 != null) {
                            if (object.ultimateAddressLine1.length > 32) {
                                ultimateAddress1 = object.ultimateAddressLine1.toString().substring(0, 31)
                                ultimateAddress2 = object.ultimateAddressLine1.toString().substring(32, object.ultimateAddressLine1.length)
                            }
                            else {
                                ultimateAddress1 = object.ultimateAddressLine1;
                            }
                        }
                        if (object.ultimateAddressLine2 != undefined && object.ultimateAddressLine2 != "" && object.ultimateAddressLine2 != null) {
                            if (object.ultimateAddressLine2.length > 32) {
                                ultimateAddress2 = object.ultimateAddressLine2.toString().substring(0, 31)

                            }
                            else {
                                ultimateAddress2 = object.ultimateAddressLine2;
                            }
                        }
                        else {
                            if (ultimateAddress2 != undefined && ultimateAddress2 != "" && ultimateAddress2 != null) {
                                if (ultimateAddress2.length > 32) {
                                    ultimateAddress2 = ultimateAddress2.toString().substring(0, 31)

                                }

                            }
                        }
                        $scope.entity.ultimateAddressLine1 = ultimateAddress1;
                        $scope.entity.ultimateAddressLine2 = ultimateAddress2;
                        ...
                        var interAddress1 = "";
                        var interAddress2 = "";
                        if (object.interAddressLine1 != undefined && object.interAddressLine1 != "" && object.interAddressLine1 != null) {
                            if (object.ultimateAddressLine1.length > 32) {
                                interAddress1 = object.interAddressLine1.toString().substring(0, 31)
                                interAddress2 = object.interAddressLine1.toString().substring(32, object.interAddressLine1.length)
                            }
                            else {
                                interAddress1 = object.interAddressLine1;
                            }
                        }
                        if (object.interAddressLine2 != undefined && object.interAddressLine2 != "" && object.interAddressLine2 != null) {
                            if (object.interAddressLine2.length > 32) {
                                interAddress2 = object.interAddressLine2.toString().substring(0, 31)

                            }
                            else {
                                interAddress2 = object.interAddressLine2;
                            }
                        }
                        else {
                            if (interAddress2 != undefined && interAddress2 != "" && interAddress2 != null) {
                                if (interAddress2.length > 32) {
                                    interAddress2 = interAddress2.toString().substring(0, 31)

                                }

                            }
                        }
                        $scope.entity.interAddressLine1 = interAddress1;
                        $scope.entity.interAddressLine2 = interAddress2;
                        ...
                        var freightAddress1 = "";
                        var freightAddress2 = "";
                        if (object.freightAddressLine1 != undefined && object.freightAddressLine1 != "" && object.freightAddressLine1 != null) {
                            if (object.freightAddressLine1.length > 32) {
                                freightAddress1 = object.freightAddressLine1.toString().substring(0, 31)
                                freightAddress2 = object.freightAddressLine1.toString().substring(32, object.freightAddressLine1.length)
                            }
                            else {
                                freightAddress1 = object.freightAddressLine1;
                            }
                        }
                        if (object.freightAddressLine2 != undefined && object.freightAddressLine2 != "" && object.freightAddressLine2 != null) {
                            if (object.freightAddressLine2.length > 32) {
                                freightAddress2 = object.freightAddressLine2.toString().substring(0, 31)

                            }
                            else {
                                freightAddress2 = object.freightAddressLine2;
                            }
                        }
                        else {
                            if (freightAddress2 != undefined && freightAddress2 != "" && freightAddress2 != null) {
                                if (freightAddress2.length > 32) {
                                    freightAddress2 = freightAddress2.toString().substring(0, 31)

                                }

                            }
                        }
                        $scope.entity.freightAddressLine1 = freightAddress1;
                        $scope.entity.freightAddressLine2 = freightAddress2;
                        ...
                        $scope.entity.exporterFirstName = object.exporterFirstName;
                        $scope.entity.exporterLastName = object.exporterLastName;
                        $scope.entity.exporterPhoneNumber = object.exporterPhoneNumber;
                        $scope.entity.exporterCountryId = object.exporterCountryId;
                        $scope.entity.exporterCountryCode = object.exporterCountryCode;
                        $scope.entity.exporterStateId = object.exporterStateId;
                        $scope.entity.exporterStateCode = object.exporterStateCode;
                        $scope.entity.exporterCity = object.exporterCity;
                        $scope.entity.exporterPostalCodeId = object.exporterPostalCodeId;
                        // $scope.entity.exporterAddress = object.exporterAddress;
                        //  $scope.entity.exporterAddress2 = object.exporterAddress2;
                        var exporterAddress1 = "";
                        var exporterAddress2 = "";
                        if (object.exporterAddress != undefined && object.exporterAddress != "" && object.exporterAddress != null) {
                            if (object.exporterAddress.length > 32) {
                                exporterAddress1 = object.exporterAddress.toString().substring(0, 31)
                                exporterAddress2 = object.exporterAddress.toString().substring(32, object.exporterAddress.length)
                            }
                            else {
                                exporterAddress1 = object.exporterAddress;
                            }
                        }
                        if (object.exporterAddress2 != undefined && object.exporterAddress2 != "" && object.exporterAddress2 != null) {
                            if (object.exporterAddress2.length > 32) {
                                exporterAddress2 = object.exporterAddress2.toString().substring(0, 31)

                            }
                            else {
                                exporterAddress2 = object.exporterAddress2;
                            }
                        }
                        else {
                            if (exporterAddress2 != undefined && exporterAddress2 != "" && exporterAddress2 != null) {
                                if (exporterAddress2.length > 32) {
                                    exporterAddress2 = exporterAddress2.toString().substring(0, 31)

                                }

                            }
                        }
                        $scope.entity.exporterAddress = exporterAddress1;
                        $scope.entity.exporterAddress2 = exporterAddress2;
                        $scope.entity.exporterEmail = object.exporterEmail;
                        $scope.entity.etype = type;
                        $scope.entity.isuploaded = object.isuploaded;
                        $scope.entity.FileList = object.fileList;
                        $scope.entity.aes_Status = object.aes_Status;
                        $scope.entity.isFreeze = object.isFreeze;
                        $scope.entity.freezeRemarks = object.freezeRemarks;
                        if ($scope.entity.originalITN != "" && $scope.entity.aes_Status != "0") {
                            $scope.hideisfreeze = false;
                            $scope.hidesave = true;
                        }
                        else {
                            $scope.HideFreezeRemarks = true;
                        ...
                        $scope.entity.dataSourceFrom = "-Select-";
                    }
                    else {
        ...
        $scope.addAesSubmission = function (sourcetype) {

            var errormsg = "";
            var errormainlist = "";
            var isvalid = 0;
            if ($scope.entity.isDraft == false && sourcetype!='T') {
                errormainlist = "<ol>"
                errormsg = "";
                if ($scope.entity.shipmentNumber == "" || $scope.entity.shipmentNumber == undefined) {
                    errormsg += "<li>Shipment Reference Number is required.</li>";

                }
                if ($scope.entity.emailResponseAddress == "" || $scope.entity.emailResponseAddress == undefined) {
                    errormsg += "<li>Email Response Address is required.</li>";

                }
                if ($scope.entity.filingOption == undefined || $scope.entity.filingOption == "") {
                    errormsg += "<li>Please Select filing option</li>";

                }
                if ($scope.entity.portofExport == undefined || $scope.entity.portofExport == "") {
                    errormsg += "<li>Please Select Port of Export.</li>";

                }
                if ($scope.entity.portofExportCode == undefined || $scope.entity.portofExportCode == null || $scope.entity.portofExportCode == "") {
                    errormsg += "<li>Port of Export Code is required.</li>";
                }
                if ($scope.entity.portofExportCode != undefined && $scope.entity.portofExportCode != null && $scope.entity.portofExportCode != "") {
                    if ($scope.entity.portofExportCode.length != 4) {
                        errormsg += "<li>Port of Export Code length should be four .</li>";
                    }
                }
                if ($scope.entity.modeofTransport == undefined || $scope.entity.modeofTransport == "") {
                    errormsg += "<li>Please Select Mode of Transport.</li>";

                }
                //if ($scope.entity.portofUnloading == undefined || $scope.entity.portofUnloading == "") {
                //    errormsg += "<li>Please Select Port of Unloading.<li>";

                //}
                //if ($scope.entity.portofUnloadingCode == undefined || $scope.entity.portofUnloadingCode == null || $scope.entity.portofUnloadingCode == "") {
                //    errormsg += "<li>Port of Unloading Code is required.</li>";
                //}
                if ($scope.entity.portofUnloadingCode != undefined && $scope.entity.portofUnloadingCode != null && $scope.entity.portofUnloadingCode != "") {
                    if ($scope.entity.portofUnloadingCode.length != 5) {
                        errormsg += "<li>Port of Unloading Code length should be five .</li>";
                    }
                }
                if ($scope.entity.departureDate == undefined || $scope.entity.departureDate == "") {
                    errormsg += "<li>Departure Date is required.</li> ";

                }
                if ($scope.entity.originState == undefined || $scope.entity.originState == "") {
                    errormsg += "<li>Please Select Origin State.</li>";

                }
                if ($scope.entity.originStateCode == undefined || $scope.entity.originStateCode == null || $scope.entity.originStateCode == "") {
                    errormsg += "<li>Origin State Code is required.</li>";
                }
                if ($scope.entity.originStateCode != undefined && $scope.entity.originStateCode != null && $scope.entity.originStateCode != "") {
                    if ($scope.entity.originStateCode.length != 2) {
                        errormsg += "<li>Origin State Code length should be two.</li>";
                    }
                }
                if ($scope.entity.countryofDestination == undefined || $scope.entity.countryofDestination == "") {
                    errormsg += "<li>Please Select Country of Destination.</li>";

                }
                if ($scope.entity.countryCode == undefined || $scope.entity.countryCode == null || $scope.entity.countryCode == "") {
                    errormsg += "<li>Country Code is required.</li>";
                }
                if ($scope.entity.countryCode != undefined || $scope.entity.countryCode != null || $scope.entity.countryCode != "") {
                    if ($scope.entity.countryCode.length != 2) {
                        errormsg += "<li>Country Code length should be two.</li>";
                    }
                }
                if ($scope.entity.inbondType == undefined || $scope.entity.inbondType == "") {
                    errormsg += "<li>Please Select Inbond Type.</li>";

                }

                if ($scope.entity.exporterName == "" || $scope.entity.exporterName == undefined) {
                    $scope.entity.exporterId = "0";
                    //errormsg += "<li>Exporter Name is required.</li>";
                }


                if ($scope.entity.isRoutedTransaction == "" || $scope.entity.isRoutedTransaction == undefined) {
                    errormsg += "<li>Is this a Routed Transaction is required.</li>";

                }
                if ($scope.entity.isUltimateConsigneeCompanies == "" || $scope.entity.isUltimateConsigneeCompanies == undefined) {
                    errormsg += "<li>Are USPPI and Ultimate Consignee Companies related? is required.</li>";

                }

                if ($scope.entity.isHazardousMaterial == "" || $scope.entity.isHazardousMaterial == undefined) {
                    errormsg += "<li>Does shipment contain hazardous material? is required.</li>";

                }
                if (errormsg != "") {

                    errormainlist += "<li style='color:red'>Shipment";
                    errormsg += "</ol>";
                    errormainlist += "<ol>" + errormsg + "</li>";
                }
                ...
                if ($scope.entity.uSPPIIDNumberTypeId == "Please Select" || $scope.entity.uSPPIIDNumberTypeId == undefined) {
                    errormsg += "<li>USPPI ID Number Type is required.</li>";

                }
                if ($scope.entity.uSPPIIDNumber == "" || $scope.entity.uSPPIIDNumber == undefined) {
                    errormsg += "<li>USPPI ID Number is required.</li>";

                }
                if ($scope.entity.uSPPICompanyName == "" || $scope.entity.uSPPICompanyName == undefined) {
                    errormsg += "<li>USPPI Company Name is required.</li>";

                }
                if ($scope.entity.uSPPIFirstName == "" || $scope.entity.uSPPIFirstName == undefined) {
                    errormsg += "<li>USPPI First Name is required.</li>";

                }
                if ($scope.entity.uSPPILastName == "" || $scope.entity.uSPPILastName == undefined) {
                    errormsg += "<li>USPPI Last Name is required.</li>";

                }
                if ($scope.entity.uSPPIPhoneNumber == "" || $scope.entity.uSPPIPhoneNumber == undefined) {
                    errormsg += "<li>USPPI Phone No is required.</li>";

                }
                if ($scope.entity.uSPPIAddressLine1 == "" || $scope.entity.uSPPIAddressLine1 == undefined) {
                    errormsg += "<li>USPPI Address line 1 is required.</li>";

                }

                if ($scope.entity.uSPPIPostalCodeId == "" || $scope.entity.uSPPIPostalCodeId == undefined) {
                    errormsg += "<li>USPPI Postal Code is required.</li>";

                }
                if ($scope.entity.uSPPICity == "" || $scope.entity.uSPPICity == undefined) {
                    errormsg += "<li>USPPI City is required.</li>";

                }
                if ($scope.entity.uSPPIStateId == "" || $scope.entity.uSPPIStateId == undefined) {
                    errormsg += "<li>USPPI State is required.</li>";

                }
                if ($scope.entity.uSPPIStateCode == undefined || $scope.entity.uSPPIStateCode == null || $scope.entity.uSPPIStateCode == "") {
                    errormsg += "<li>USPPI State Code is required.</li>";
                }
                // Ultimate Consignee
                if ($scope.entity.isSoldEnRoute == "" || $scope.entity.isSoldEnRoute == undefined) {
                    errormsg += "<li>Sold En Route? is required.</li>";

                }
                if ($scope.entity.consigneeTypeId == "" || $scope.entity.consigneeTypeId == undefined) {
                    errormsg += "<li>Consignee Type is required.</li>";

                }

                if ($scope.entity.ultimateCompanyName == "" || $scope.entity.ultimateCompanyName == undefined) {
                    errormsg += "<li>Ultimate Consignee Company Name is required.</li>";

                }
                //if ($scope.entity.ultimateFirstName == "" || $scope.entity.ultimateFirstName == undefined) {
                //    errormsg += "<li>Ultimate First Name is required.</li>";

                //}
                //if ($scope.entity.ultimateLastName == "" || $scope.entity.ultimateLastName == undefined) {
                //    errormsg += "<li>Ultimate Consignee Last Name is required.</li>";

                //}
                //if ($scope.entity.ultimatePhoneNumber == "" || $scope.entity.ultimatePhoneNumber == undefined) {
                //    errormsg += "<li>Ultimate Consignee Phone No is required.</li>";

                //}
                if ($scope.entity.ultimateAddressLine1 == "" || $scope.entity.ultimateAddressLine1 == undefined) {
                    errormsg += "<li>Ultimate Consignee Address line 1 is required.</li>";

                }

                if ($scope.entity.ultimateCountryId == "" || $scope.entity.ultimateCountryId == undefined) {
                    errormsg += "<li>Ultimate Consignee Country is required.</li>";

                }
                //if ($scope.entity.ultimatePostalCodeId == "" || $scope.entity.ultimatePostalCodeId == undefined) {
                //    errormsg += "<li>Ultimate Consignee Postal Code is required.</li>";
                //    //return;
                //}
                if ($scope.entity.ultimateCity == "" || $scope.entity.ultimateCity == undefined) {
                    errormsg += "<li>Ultimate Consignee City is required.</li>";

                }
                //if ($scope.entity.ultimateStateId == "" || $scope.entity.ultimateStateId == undefined) {
                //    errormsg += "<li>Ultimate Consignee State is required.</li>";

                //}
                if ($scope.entity.ultimateCountryCode == undefined || $scope.entity.ultimateCountryCode == null || $scope.entity.ultimateCountryCode == "") {
                    errormsg += "<li>Ultimate Country Code is required.</li>";
                }
                //if ($scope.entity.ultimateStateCode == undefined || $scope.entity.ultimateStateCode == null || $scope.entity.ultimateStateCode == "") {
                //    errormsg += "<li>Ultimate State Code is required.</li>";
                //}

                //intermediate

                //if ($scope.entity.interIdNumberTypeId == "Please Select" || $scope.entity.interIdNumberTypeId == undefined) {
                //    errormsg += "<li>Intermediate Consignee ID Number Type is required.</li>";
                //    //return;
                //}
                //if ($scope.entity.interIdNumber == "" || $scope.entity.interIdNumber == undefined) {
                //    errormsg += "<li>Intermediate Consignee ID Number is required.</li>";
                //    //return;
                //}
                //if ($scope.entity.interCompanyName == "" || $scope.entity.interCompanyName == undefined) {
                //    errormsg += "<li>Intermediate Consignee Company Name is required.</li>";
                //    //return;
                //}
                //if ($scope.entity.interFirstName == "" || $scope.entity.interFirstName == undefined) {
                //    errormsg += "<li>Intermediate Consignee First Name is required.</li>";
                //   // return;
                //}
                //if ($scope.entity.interLastName == "" || $scope.entity.interLastName == undefined) {
                //    errormsg += "<li>Intermediate Consignee Last Name is required.</li>";
                //   // return;
                //}
                //if ($scope.entity.interPhoneNumber == "" || $scope.entity.interPhoneNumber == undefined) {
                //    errormsg += "<li>Intermediate Consignee Phone No is required.</li>";
                //    //return;
                //}
                //if ($scope.entity.interAddressLine1 == "" || $scope.entity.interAddressLine1 == undefined) {
                //    errormsg += "<li>Intermediate Consignee Address line 1 is required.</li>";
                //   // return;
                //}

                //if ($scope.entity.interCountryId == "" || $scope.entity.interCountryId == undefined) {
                //    errormsg += "<li>Intermediate Consignee Country is required.</li>";
                //    //return;
                //}
                //if ($scope.entity.interPostalCodeId == "" || $scope.entity.interPostalCodeId == undefined) {
                //    errormsg += "<li>Intermediate Consignee Postal Code is required.</li>";
                //    //return;
                //}
                //if ($scope.entity.interCity == "" || $scope.entity.interCity == undefined) {
                //    errormsg += "<li>Intermediate Consignee City is required.</li>";
                //   // return;
                //}
                //if ($scope.entity.interStateId == "" || $scope.entity.interStateId == undefined) {
                //    errormsg += "<li>Intermediate Consignee State is required.</li>";
                //    //return;
                //}

                //if ($scope.entity.interCountryCode == undefined || $scope.entity.interCountryCode == null || $scope.entity.interCountryCode == "") {
                //    errormsg += "<li>Intermediate Country Code is required.</li>";
                //}
                //if ($scope.entity.interStateCode == undefined || $scope.entity.interStateCode == null || $scope.entity.interStateCode == "") {
                //    errormsg += "<li>Intermediate State Code is required.</li>";
                //}

                //Freight Forwarder

                //if ($scope.entity.freightIdNumberTypeId == "Please Select" || $scope.entity.freightIdNumberTypeId == undefined) {
                //    errormsg += "<li>Freight Forwarder ID Number Type is required.</li>";

                //}
                //if ($scope.entity.freightIdNumber == "" || $scope.entity.freightIdNumber == undefined) {
                //    errormsg += "<li>Freight Forwarder ID Number is required.</li>";

                //}
                //if ($scope.entity.freightCompanyName == "" || $scope.entity.freightCompanyName == undefined) {
                //    errormsg += "<li>Freight Forwarder Company Name is required.</li>";

                //}
                //if ($scope.entity.freightFirstName == "" || $scope.entity.freightFirstName == undefined) {
                //    errormsg += "<li>Freight Forwarder First Name is required.</li>";

                //}
                //if ($scope.entity.freightLastName == "" || $scope.entity.freightLastName == undefined) {
                //    errormsg += "<li>Freight Forwarder Last Name is required.</li>";

                //}
                //if ($scope.entity.freightPhoneNumber == "" || $scope.entity.freightPhoneNumber == undefined) {
                //    errormsg += "<li>Freight Forwarder Phone No is required.</li>";

                //}
                //if ($scope.entity.freightAddressLine1 == "" || $scope.entity.freightAddressLine1 == undefined) {
                //    errormsg += "<li>Freight Forwarder Address line 1 is required.</li>";

                //}

                //if ($scope.entity.freightPostalCodeId == "" || $scope.entity.freightPostalCodeId == undefined) {
                //    errormsg += "<li>Freight Forwarder Postal Code is required.</li>";

                //}
                //if ($scope.entity.freightCity == "" || $scope.entity.freightCity == undefined) {
                //    errormsg += "<li>Freight Forwarder City is required.</li>";

                //}
                //if ($scope.entity.freightStateId == "" || $scope.entity.freightStateId == undefined) {
                //    errormsg += "<li>Freight Forwarder state is required.</li>";

                //}
                //if ($scope.entity.freightStateCode == undefined || $scope.entity.freightStateCode == null || $scope.entity.freightStateCode == "") {
                //    errormsg += "<li>Freight Forwarder State Code is required.</li>";
                //}
                //if ($scope.entity.conveyanceName == "" || $scope.entity.conveyanceName == undefined) {
                //    errormsg += "<li>Freight Forwarder email is required.</li>";

                //}

                if (errormsg != "") {

                    errormainlist += "<li style='color:red'>Parties";
                    errormsg += "</ol>";
                    errormainlist += "<ol>" + errormsg + "</li>";
                }
                errormsg = "";
                ...
                //    errormsg += "<li>Carrier SCAC/IATA is required.</li>";

                //}
                //if ($scope.entity.conveyanceName == "" || $scope.entity.conveyanceName == undefined) {
                //    errormsg += "<li>Conveyance Name/Carrier Name is required.</li>";

                //}
                //if ($scope.entity.transportationReferenceNumber == "" || $scope.entity.transportationReferenceNumber == undefined) {
                //    errormsg += "<li>Transportation Reference Number Name is required.</li>";

                //}
                if (errormsg != "") {

                    errormainlist += "<li style='color:red'>Transportation";
                    errormsg += "</ol>";
                    errormainlist += "<ol>" + errormsg + "</li>";
                }
                errormsg = "";
                ...
                if ($scope.entity.commodityDetail.length > 0) {
                        ...
                        if ($scope.entity.commodityDetail[i].siteId != "0") {
                            if ($scope.entity.commodityDetail[i].isType != "D") {
                                ...
                                if ($scope.entity.commodityDetail[i].exportCode == '-' || $scope.entity.commodityDetail[i].exportCode == '-Select-' || $scope.entity.commodityDetail[i].exportCode == '' || $scope.entity.commodityDetail[i].exportCode == undefined) {
                                    ...
                                    errormsg += "<li>Export Information Code is required in Line item " + j + ".</li>";
                                }
                                //if ($scope.entity.commodityDetail[i].scheduleB == '') {

                                //    errormsg += "<li>Schedule B or HTS Number is required in Line item " + j + ".</li>";
                                //}
                                if ($scope.entity.commodityDetail[i].firstQuantity == '') {

                                    errormsg += "<li>First Quantity is required in Line item " + j + ".</li>";
                                }
                                if ($scope.entity.commodityDetail[i].firstUOM == '-Select-' || $scope.entity.commodityDetail[i].firstUOM == '' || $scope.entity.commodityDetail[i].firstUOM == undefined) {
                                    ...
                                    errormsg += "<li>First UOM is required in Line item " + j + ".</li>";
                                }
                                ...
                                if ($scope.entity.commodityDetail[i].originofGoods == '-Select-' || $scope.entity.commodityDetail[i].originofGoods == '' || $scope.entity.commodityDetail[i].originofGoods == undefined) {
                                    ...
                                    errormsg += "<li>Origin of Goods is required in Line item " + j + ".</li>";
                                }
                                if ($scope.entity.commodityDetail[i].valueofGoods == '') {

                                    errormsg += "<li>Value of Goods is required in Line item " + j + ".</li>";
                                }
                                //if ($scope.entity.commodityDetail[i].shippingWeight == '') {

                                //    errormsg += "<li>Shipping Weight of is required in Line item " + j + ".</li>";
                                //}
                                ...
                                if ($scope.entity.commodityDetail[i].licenseTypeCode == '-Select-' || $scope.entity.commodityDetail[i].licenseTypeCode == '' || $scope.entity.commodityDetail[i].licenseTypeCode == undefined) {
                                    ...
                                    errormsg += "<li>License Type Code is required in Line item " + j + ".</li>";
                                }
                                if ($scope.entity.commodityDetail[i].isGovermentAgency == '' || $scope.entity.commodityDetail[i].isGovermentAgency == undefined) {

                                    errormsg += "<li>Does this filing require Participating Goverment agency data is required in Line item " + j + ".</li>";
                                }
                            }
                        }
                    ...
                    if (isdel == 0) {
                        errormsg += "<li>Please add atleast one commodity item detail</li>";
                    }
                }
                else {
                    errormsg += "<li>Please add atleast one commodity item detail</li>";
                ...
                if (errormsg != "") {
                    ...
                    errormainlist += "<li style='color:red'>Commodity";
                    errormsg += "</ol>";
                    errormainlist += "<ol>" + errormsg + "</li>";
                }

                if (errormainlist != "" && errormainlist != "<ol>") {
                    $scope.messagelist = errormainlist;
                    alertMessage(errormainlist);
                    return;
                }
                ...
                if ($scope.entity.portofUnloading == undefined || $scope.entity.portofUnloading == "") {
                    $scope.entity.portofUnloadingCode = "";
                }
            }
            //if ($scope.entity.aestype == 'A') {
            //for (var i = 0; i < $scope.entity.commodityDetail.length; i++) {
            //        $scope.entity.commodityDetail[i].ctype ='A';
            //    }
            //}

            if (sourcetype == 'T') {
                $scope.entity.isTemplate = "1";
            }

            if ($('#originalITN').val() != undefined)
                $scope.entity.originalITN = $('#originalITN').val();
        ...
        $scope.selectTypeaheadAES = function ($item, $model, $label, source, stype) {
            ...
            if (stype == "portofExport") {
                ...
                $scope.entity.portId = $item.portId;
            }
            if (stype == "portofUnloading") {
                $scope.entity.portofUnloadingCode = $item.id;
                $scope.entity.unloadPortId = $item.portId;
            }
            ...
            if (stype == "exporterCountryCode") {
                $scope.entity.exporterCountryCode = $item.id;
            }
            if (stype == "exporterStateCode") {
                $scope.entity.exporterStateCode = $item.id;
            }
        ...
        $scope.setLookups = function (source, lookup, output, index) {
            ...
            if (lookup == "SIPLContact1") {
                var ultimateAddress1 = "";
                var ultimateAddress2 = "";
                if (output.data[0].address != undefined && output.data[0].address != "" && output.data[0].address != null) {
                    if (output.data[0].address.length > 32) {
                        ultimateAddress1 = output.data[0].address.toString().substring(0, 31)
                        ultimateAddress2 = output.data[0].address.toString().substring(32, output.data[0].address.length)
                    }
                    else {
                        ultimateAddress1 = output.data[0].address
                    }
                }
                $scope.entity.ultimateAddressLine1 = ultimateAddress1;
                $scope.entity.ultimateAddressLine2 = ultimateAddress2;
                ...
                $scope.entity.ultimateCountryCode = output.data[0].countryCode;
                ...
                $scope.entity.ultimateStateCode = output.data[0].stateCode;
            ...
            if (lookup == "SIPLContact2") {
                var interAddressAddress1 = "";
                var interAddressAddress2 = "";
                if (output.data[0].address != undefined && output.data[0].address != "" && output.data[0].address != null) {
                    if (output.data[0].address.length > 32) {
                        interAddressAddress1 = output.data[0].address.toString().substring(0, 31)
                        interAddressAddress2 = output.data[0].address.toString().substring(32, output.data[0].address.length)
                    }
                    else {
                        interAddressAddress1 = output.data[0].address
                    }
                }
                $scope.entity.interAddressLine1 = interAddressAddress1
                $scope.entity.interAddressLine2 = interAddressAddress2
                if (output.data[0].contactPerson != "" && output.data[0].contactPerson != undefined) {
                ...
                $scope.entity.interCountryCode = output.data[0].countryCode;
                ...
                $scope.entity.interStateCode = output.data[0].stateCode;
            ...
            if (lookup == "SIPLContact3") {
                ...
                $scope.entity.exporterCountryId = output.data[0].countryName;
                $scope.entity.exporterCountryCode = output.data[0].countryCode;
                $scope.entity.exporterEmail = output.data[0].email;
                $scope.entity.exporterPostalCodeId = output.data[0].zipCode;
                $scope.entity.exporterCity = output.data[0].cityName;
                $scope.entity.exporterStateId = output.data[0].stateName;
                $scope.entity.exporterStateCode = output.data[0].stateCode;
                $scope.entity.exporterEmail = output.data[0].email;
            }
            if (lookup == "SIPLContact5") {
                var uSPPIAddress1 = "";
                var uSPPIAddress2 = "";
                if (output.data[0].address != undefined && output.data[0].address != "" && output.data[0].address != null) {
                    if (output.data[0].address.length > 32) {
                        uSPPIAddress1 = output.data[0].address.toString().substring(0, 31)
                        uSPPIAddress2 = output.data[0].address.toString().substring(32, output.data[0].address.length)
                    }
                    else {
                        uSPPIAddress1 = output.data[0].address
                    }
                }
                $scope.entity.uSPPIAddressLine1 = uSPPIAddress1;
                $scope.entity.uSPPIAddressLine2 = uSPPIAddress2;
                ...
                if (output.data[0].contactPerson != "" && output.data[0].contactPerson != undefined) {
                    $scope.entity.uSPPIFirstName = output.data[0].contactPerson.split(" ")[0];
                    $scope.entity.uSPPILastName = output.data[0].contactPerson.split(" ")[1];
                }

                $scope.entity.uSPPIPhoneNumber = output.data[0].telNo.replace('|', '').replace('-', '');
                $scope.entity.uSPPICountryId = output.data[0].countryName;
                $scope.entity.uSPPICountryCode = output.data[0].countryCode;
                $scope.entity.uSPPIPostalCodeId = output.data[0].zipCode;
                $scope.entity.uSPPICity = output.data[0].cityName;
                $scope.entity.uSPPIStateId = output.data[0].stateName;
                $scope.entity.uSPPIStateCode = output.data[0].stateCode;
                $scope.entity.uSPPIEmail = output.data[0].email;
                $scope.entity.uSPPIIDNumber = output.data[0].idNumber;
                $scope.entity.uSPPIIDNumberTypeId = output.data[0].idNumberType;
                $scope.entity.irsNumber = output.data[0].irsNumber;
            }
        ...
        $scope.DownloadEssFile = function (documentCommonID, aesFileName) {
            ...
            entityService.DownloadEssFile(documentCommonID, aesFileName).then(
                ...
                function (output) {
                    alert("File not available please wail..");
                }
            );
        };
            ...
            entityService.GetEssFileList(documentCommonID).then(
                function (output) {
                    ...
                    modalInstance.result.then(
                        function (output) {
                            if (output != undefined) {
                                $scope.showViewDocumentModel(output.documentCommonID, 'E', output.Id)
                            }
                        },
                        function (output) {
                            ngNotifier.logError(output);
        ...
        $scope.callPortModal = function (source) {
            ...
            if (source == 'origin') {
                if ($scope.entity.portId != null) {
                    sourceId = $scope.entity.portId;

                }
            }
            if (source == 'discharge') {
                if ($scope.entity.unloadPortId != null) {
                    sourceId = $scope.entity.unloadPortId;
                    ...
                    if (output.resultId == 1001) {
                        ...
                        if (source == 'origin') {
                            if (output.data.portId != null) {
                                $scope.entity.portId = output.data.portId;
                            }
                            if (output.data.name != null) {
                                $scope.entity.portofExport = output.data.name + '-' + output.data.port_code;
                                $scope.entity.portofExportCode = output.data.port_code;
                            }
                        }
                        else {
                            if (output.data.portId != null) {
                                $scope.entity.unloadPortId = output.data.portId;
                            }
                            if (output.data.name != null) {
                                $scope.entity.portofUnloading = output.data.name + '-' + output.data.port_code;
                                $scope.entity.portofUnloadingCode = output.data.port_code;
                            }
                        }
                    }
                },
            ...
            modalInstance.result.then(
                function (output) {
                    if (output.resultId == 1001) {
                        ...
                        if (source == 'countryDest') {

                            if (output.data.name != null) {
                                $scope.entity.countryofDestination = output.data.name + '-' + output.data.countryCode;
                            }
                            if (output.data.countryCode != null) {
                                $scope.entity.countryCode = output.data.countryCode;
                            }
                        }
                ...
                function (output) {
                    if (output.resultId == 1001) {
                        if (source == 'originState') {
                            ...
                            if (output.data.name != null) {
                                $scope.entity.originState = output.data.name + '-' + output.data.stateCode;
                            }
                            if (output.data.stateCode != null) {
                                $scope.entity.stateCode = output.data.stateCode;
                            }
                        }
        ...
        $scope.hideshowfreezeremark = function (isFreeze) {

            if ($scope.hidesave1 == 1) {
                $scope.hidesave = true;
                $scope.HideFreezeRemarks = true;
            }
            else if (isFreeze == "0" && $scope.hidesave == false) {
                $scope.hidesave = true;
            }
            else {
                $scope.hidesave = false;
            ...
            if (isFreeze == "2") {
                $scope.hidesave = false;
                $scope.HideFreezeRemarks = false;
            }

        };
        $scope.pdfreportdownload = function (id) {

            var reportParams = {
                id: id,
                documentcommanid: $scope.entity.documentCommonID,
                SitId: $scope.$parent.selectedSiteId,
                createdby: $scope.$parent.authentication.userId
            };
            entityService.pdfreportdownload(reportParams).then(
                function (output) {
                    var blobData = new Blob([output.data], { type: output.headers()["content-type"] });
                    var fileName = output.headers()["x-filename"];
                    saveAs(blobData, fileName);
        ...
        $scope.isaddchangereplace = function (changetype) {
            if (changetype == "X") {
                $scope.entity.isuploaded == 0
            }
        }
        $scope.viewtemplatedata = function (siteId, documentCommonID) {
           // entityService.GetTemplateList(siteId).then(
             //   function (output) {

                   // $scope.templateList = output.data.data;
                    var modalInstance = $uibModal.open({
                        animation: false,
                        backdrop: "static",
                        keyboard: false,
                        size: "lg",
                        templateUrl: "app/components/ees/eesSub/templatedata.html",
                        controller: "templatedataController",
                        resolve: {
                            requestData: function () {
                                return {
                                    siteId: $scope.$parent.selectedSiteId,
                    ...
                    modalInstance.result.then(
                        function (output) {
                            if (output != undefined) {
                                if (output.dcommonid != undefined) {
                                    $scope.showViewDocumentModel(output.dcommonid, 'T', output.id)
                                }
                            }
                        },
                        function (output) {
        ...
        $scope.viewpartytemplate = function (dform) {
          var modalInstance = $uibModal.open({
                        animation: false,
                        backdrop: "static",
                        keyboard: false,
                        size: "lg",
                        templateUrl: "app/components/ees/eesSub/partytemplatedata.html",
                        controller: "templatedataController",
                        resolve: {
                            requestData: function () {
                                return {
          ...
          modalInstance.result.then(
                        function (output) {
                            if (output != undefined) {
                                if ($scope.entity.dataSourceFrom == 'USPPI' || output.templateType == 'USPPI') {
                                    $scope.entity.uSPPICompanyName = output.companyName;
                                    $scope.entity.uSPPIFirstName = output.firstName;
                                    $scope.entity.uSPPILastName = output.lastName;
                                    $scope.entity.uSPPIPhoneNumber = output.phoneNumber;
                                    $scope.entity.uSPPIAddressLine1 = output.addressLine1;
                                    $scope.entity.uSPPIAddressLine2 = output.addressLine2;
                                    $scope.entity.uSPPIPostalCodeId = output.postalCode;
                                    $scope.entity.uSPPICity = output.cityName;
                                    $scope.entity.uSPPIStateId = output.stateName;
                                    $scope.entity.uSPPIStateCode = output.stateCode;
                                    $scope.entity.uSPPIIDNumber = output.einNo;
                                    $scope.entity.uSPPIIDNumberTypeId = "E";
                                }
                                else {
                                    $scope.entity.ultimateCompanyName = output.companyName;
                                    $scope.entity.ultimateFirstName = output.firstName;
                                    $scope.entity.ultimateLastName = output.lastName;
                                    $scope.entity.ultimatePhoneNumber = output.phoneNumber;
                                    $scope.entity.ultimateAddressLine1 = output.addressLine1;
                                    $scope.entity.ultimateAddressLine2 = output.addressLine2;
                                    $scope.entity.ultimatePostalCodeId = output.postalCode;
                                    $scope.entity.ultimateCountryId = output.countryName;
                                    $scope.entity.ultimateCountryCode = output.countryCode;
                                    $scope.entity.ultimateCity = output.cityName;
                                    $scope.entity.ultimateStateId = output.stateName;
                                    $scope.entity.ultimateStateCode = output.stateCode;
        ...
        $scope.saveAesTemplate = function (templateType) {
            $scope.entitys = {};
            var errormsg = "";
            $scope.$broadcast("show-errors-check-validity");
            if (templateType != 'Ultimate') {
                ...
                $scope.entitys.companyName = $scope.entity.uSPPICompanyName;
                $scope.entitys.firstName = $scope.entity.uSPPIFirstName;
                $scope.entitys.lastName = $scope.entity.uSPPILastName;
                $scope.entitys.phoneNumber = $scope.entity.uSPPIPhoneNumber;
                $scope.entitys.addressLine1 = $scope.entity.uSPPIAddressLine1;
                $scope.entitys.addressLine2 = $scope.entity.uSPPIAddressLine2;
                $scope.entitys.countryName = "";
                $scope.entitys.countryId = "0";
                $scope.entitys.countryCode = "";
                $scope.entitys.postalCode = $scope.entity.uSPPIPostalCodeId;
                $scope.entitys.cityName = $scope.entity.uSPPICity;
                $scope.entitys.stateName = $scope.entity.uSPPIStateId;
                $scope.entitys.stateId = "0";
                $scope.entitys.stateCode = $scope.entity.uSPPIStateCode;
                $scope.entitys.einNo = $scope.entity.uSPPIIDNumber;
            }
            else {
                if ($scope.entity.ultimateCompanyName == "" || $scope.entity.ultimateCompanyName == undefined) {
                    errormsg += "<li>Company Name is required.</li>";
                ...
                $scope.entitys.companyName = $scope.entity.ultimateCompanyName;
                $scope.entitys.firstName = $scope.entity.ultimateFirstName;
                $scope.entitys.lastName = $scope.entity.ultimateLastName;
                $scope.entitys.phoneNumber = $scope.entity.ultimatePhoneNumber;
                $scope.entitys.addressLine1 = $scope.entity.ultimateAddressLine1;
                $scope.entitys.addressLine2 = $scope.entity.ultimateAddressLine2;
                $scope.entitys.countryName = $scope.entity.ultimateCountryId;
                $scope.entitys.countryId = "0";
                $scope.entitys.countryCode = $scope.entity.UltimateCountryCode;
                $scope.entitys.postalCode = $scope.entity.ultimatePostalCodeId;
                $scope.entitys.cityName = $scope.entity.ultimateCity;
                $scope.entitys.stateName = $scope.entity.ultimateStateId;
                $scope.entitys.stateId = "0";
                $scope.entitys.stateCode = $scope.entity.UltimateStateCode;
                $scope.entitys.einNo = "";
            }
            if (errormsg != "") {
            ...
            $scope.entitys.templateType = templateType;
            $scope.entitys.isActive = "true";
            $scope.entitys.createdBy = $scope.$parent.authentication.userId;
            entityService.saveAesTemplate($scope.entitys).then(
                function (output) {
                    $scope.entitys = {};
                    ngNotifier.show(output.data);
                },
